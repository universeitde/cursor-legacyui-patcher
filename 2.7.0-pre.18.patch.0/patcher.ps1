<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.18 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.18.
    Pre.18 variables: xPw (2 layouts, Zen+Browser injected via spread), rh (clear),
    C( (i18n), q0 (layout enum), h4/WDw (opener), nL (Cursor Settings cmd), Ti (StorageScope).
#>
...
if ([string]::IsNullOrWhiteSpace($Path)) {
    ...
    if ($installs.Count -eq 0) {
        Write-Host "ERROR: No Cursor installation found." -ForegroundColor Red
        exit 1
    }
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:H=>{try{const G=h4(a,u)?_:void 0;return i.createInstance(RPw'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:H=>{try{const G=h4(a,u)?_:void 0;return i.createInstance(RPw'
    },
    @{
        Name                = "buildContent_full_pre18"
        AllowLengthMismatch = $true
...
function Update-ProductJsonChecksum {
    ...
    if ($new -eq $content) { Write-ColorOutput "WARNING: Could not update $ChecksumKey" "Yellow"; return $false }
    [IO.File]::WriteAllText($ProductJsonFile, $new, [Text.UTF8Encoding]::new($false))
    $true
}
function Update-ProductJsonChecksumWithFallback {
    param($ChecksumKeys, $NewChecksum, $DisplayName)
...
function Invoke-Restore {
    ...
    if (Test-Path $NlsMessagesFile) {
        $nls = Get-LatestBackup (Split-Path $NlsMessagesFile -Parent) $NlsBackupPattern
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.18) ===" "Cyan"
    ...
    try {
        ...
        if (Test-Path $NlsMessagesFile) {
            ...
            if ($nls.Contains($NlsCorruptPatch.Search)) {
                ...
                [IO.File]::WriteAllText($NlsMessagesFile, $nls, [Text.UTF8Encoding]::new($false))
