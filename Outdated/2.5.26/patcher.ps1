...
# search/replace kept same length for source maps
...
function Get-WorkbenchBackupWithPatchTargets {
    ...
    foreach ($backup in $backups) {
        try {
            ...
            if ($content.Contains("hide_layout_extended") -and $content.Contains("shouldHideExtendedLayoutControls")) {
                return $backup
            }
        }
        catch {
...
function Invoke-Patch {
    ...
    if ($alreadyApplied.Count -eq $Patches.Count -and -not $bootstrapNeedsFix) {
        Write-ColorOutput ""
    ...
    if ($alreadyApplied.Count -eq $Patches.Count -and $bootstrapNeedsFix) {
        Write-ColorOutput ""
    ...
    # search/replace same length (srcmaps)
    foreach ($p in $patchesToApply) {
        if ($p.Search.Length -ne $p.Replace.Length) {
