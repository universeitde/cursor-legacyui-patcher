<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.183 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.183.
    i18n E(). WxS (layouts), Ah (clear), XB (layoutEnum), Y2 (cmd), jxS (createInstance), Si (StorageScope), opener kR.
    Zen E(3212), Browser E(3092); avoid E(3091) Layout+Zen. Agent/Editor E(3070)/E(3071). Settings E(3067).
    Opener: getAnchor ()=>O, layer:-24 removed; createInstance(jxS,...).render(U) tail (pre.183 shape).
#>
...
$Patches = @(
    ...
    @{
        Name                = "opener_no_layer_pre183"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=kR(a,u)?S:void 0;return i.createInstance(jxS,s,o,a,l,u,d,h,f,p,g,i,_,q,N,k).render(U)}catch(q){throw q}}'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=kR(a,u)?S:void 0;return i.createInstance(jxS,s,o,a,l,u,d,h,f,p,g,i,_,q,N,k).render(U)}catch(q){throw q}}'
    },
    @{
        Name                = "buildContent_full_pre183"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.183) ===" "Cyan"
