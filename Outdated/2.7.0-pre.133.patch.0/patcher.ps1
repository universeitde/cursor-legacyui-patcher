<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.133 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.133.
    pre.133 variables: M__ (2 layouts), Ch (clear), VM (layoutEnum), zN (cmd), q__ (createInstance), bi (StorageScope), RI (openerCheck).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=3031, Editor=3032, Settings=3028, Zen=3275, Browser=3276.
    getAnchor var=$, render callback=q, opener var=J.
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:q=>{try{const J=RI(a,u)?_:void 0;return i.createInstance(q__'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:q=>{try{const J=RI(a,u)?_:void 0;return i.createInstance(q__'
    },
    @{
        Name                = "buildContent_full_pre133"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.133) ===" "Cyan"
