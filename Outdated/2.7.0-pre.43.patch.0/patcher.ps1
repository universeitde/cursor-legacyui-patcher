<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.43 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.43.
    Pre.43 variables: VFw (2 layouts), lh (clear), p4/e4w (opener), pL (cmd), Si (StorageScope), z0 (layoutEnum).
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:q=>{try{const G=p4(a,u)?_:void 0;return i.createInstance(e4w'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:q=>{try{const G=p4(a,u)?_:void 0;return i.createInstance(e4w'
    },
    @{
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(2939,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(pL).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.43) ===" "Cyan"
