...
$Patches = @(
    ...
    @{
        ...
        Replace = 'a.appendChild(this.createDivider());a.appendChild(this.createToggleRow({label:"Inline Diffs",getValue:()=>{const st=this.storageService??this._storageService;return st?!st.getBoolean("cursor/noInlineDiffs",-1,!1):!1},onToggle:s=>{const st=this.storageService??this._storageService;st&&st.store("cursor/noInlineDiffs",!s,-1,0)}}));if(this.isTitlebarVisibilityControlEnabled()){const s=this.createToggleRow({label:"Title Bar",getValue:()=>this.getTitlebarVisibility()==="show",onToggle:async c=>{const h=c?"show":"hide";await this.setTitlebarVisibilityPreference(h)}});this.titlebarVisibilityWrapperElement=s,a.appendChild(s)}a.appendChild(this.createToggleRow({label:"Status Bar"'
    }
)
