<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.145 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.145.
    pre.145 variables: IE_ (2 layouts), kh (clear), d2 (layoutEnum), uM (cmd), LE_ (createInstance), Ai (StorageScope), GD (openerCheck).
    ...
    getAnchor var=U, render callback=$, opener var=G.
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>U,anchorAlignment:1,anchorPosition:0,layer:-24,render:$=>{try{const G=GD(a,u)?_:void 0;return i.createInstance(LE_'
        Replace = '{getAnchor:()=>U,anchorAlignment:1,anchorPosition:0,render:$=>{try{const G=GD(a,u)?_:void 0;return i.createInstance(LE_'
    },
    @{
        Name                = "buildContent_full_pre145"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.145) ===" "Cyan"
