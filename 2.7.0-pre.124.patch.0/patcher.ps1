<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.124 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.124.
    pre.124 variables: WA_ (2 layouts), Sh (clear), iF (layoutEnum), iM (cmd), KA_ (createInstance), yi (StorageScope), BI (openerCheck).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=3031, Editor=3032, Settings=3028, Zen=3272, Browser=3273.
    getAnchor var=q, render callback=$, opener var=J.
    Stock buildContent ends with appendCursorSettingsButton + appendGlassWindowButton; the expanded Replace MUST keep
    appendGlassWindowButton(n) last so "Open Glass Window" / glass UI stays available.
    isAgentLayoutActive/isEmptyWindow/noInlineDiffs not in class.
#>
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.124) ===" "Cyan"
