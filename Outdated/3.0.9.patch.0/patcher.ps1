<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 3.0.9 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 3.0.9.
    i18n C() (not E). s$_ (layouts), xh (clear), eF (layoutEnum), YM (settings cmd), Ai (StorageScope), opener AD + u$_.
    Zen C(3211), Browser C(3091); avoid C(3090) Layout+Zen. Agent/Editor C(3069)/C(3070). Settings label C(3066) in stock footer.
    Opener: showContextView removes layer:-24 from anchor (3.x uses .render(U) tail).
#>
...
$Patches = @(
    ...
    @{
        Name                = "opener_no_layer_309"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const G=AD(a,u)?_:void 0;return i.createInstance(u$_,s,o,a,l,u,d,m,g,p,f,i,y,G,N,k).render(U)}catch(G){throw G}}'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:U=>{try{const G=AD(a,u)?_:void 0;return i.createInstance(u$_,s,o,a,l,u,d,m,g,p,f,i,y,G,N,k).render(U)}catch(G){throw G}}'
    },
    @{
        Name                = "buildContent_full_309"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_309"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(3066,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(YM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(YM);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(YM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (3.0.9) ===" "Cyan"
