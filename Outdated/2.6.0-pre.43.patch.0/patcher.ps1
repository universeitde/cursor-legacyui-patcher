...
$Patches = @(
    ...
    @{
        # Remove layer:-24 from showContextView so the context-view anchors near the gear icon.
        # With layer:-24 the context-view renders as a full-viewport overlay at top:0,left:0.
        Name                = "opener_no_layer"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:H=>{try{const W=FF(a,u)?C:void 0;return i.createInstance(BAw'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:H=>{try{const W=FF(a,u)?C:void 0;return i.createInstance(BAw'
    },
    @{
        # Restore full toggle rows. xi (not Bi) for layoutService.isVisible in pre.43.
        Name                = "buildContent_full_pre43"
        AllowLengthMismatch = $true
