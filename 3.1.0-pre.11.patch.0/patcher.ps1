<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 3.1.0-pre.11 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 3.1.0-pre.11.
    i18n E(). yIS (layouts), Eh (clear), eF (layoutEnum), X2 (settings cmd), wi (StorageScope), opener _R + kIS.
    Zen E(3093), Browser E(3094) (pre.11 nls: 3212=Tab Bar, 3092=Layout Right — do not reuse old indices). Agent/Editor E(3072)/E(3073). Settings E(3069).
    Opener: getAnchor ()=>O (letter O, not $), layer:-24 removed; createInstance(kIS,...).render(U); tail args M,x.
    Add-layout tile: show when !isApplyingLayout and under tile cap (no n.type==="unsaved" gate; matches older UX).
#>
...
$Patches = @(
    ...
    @{
        Name                = "opener_no_layer_311p11"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=_R(a,u)?S:void 0;return i.createInstance(kIS,s,o,a,l,u,d,h,f,p,g,i,_,q,M,x).render(U)}catch(q){throw q}}'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=_R(a,u)?S:void 0;return i.createInstance(kIS,s,o,a,l,u,d,h,f,p,g,i,_,q,M,x).render(U)}catch(q){throw q}}'
    },
    @{
        Name                = "buildContent_full_311p11"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_311p11"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=E(3069,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(X2).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(X2);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(X2).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (3.1.0-pre.11) ===" "Cyan"
