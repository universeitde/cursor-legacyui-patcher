<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.6.19 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.6.19.
    2.6.19 variables: eww (2 layouts), th (clear), UF/sww (opener), WP (cmd), wi (StorageScope), M0 (layoutEnum).
    Note: Uses _() for i18n (not C()), and getSidebarLocation() in renderModeGrid.
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:H=>{try{const W=UF(a,u)?C:void 0;return i.createInstance(sww'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:H=>{try{const W=UF(a,u)?C:void 0;return i.createInstance(sww'
    },
    @{
        Name                = "buildContent_full_2619"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_2619"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=_(2939,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(WP).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(WP);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(WP).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.6.19) ===" "Cyan"
