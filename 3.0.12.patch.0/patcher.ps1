<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 3.0.12 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 3.0.12.
    i18n C(). O$_ (layouts), xh (clear), X2 (layoutEnum), ZM (settings cmd), yi (StorageScope), opener AD + G$_.
    Zen C(3211), Browser C(3091); avoid C(3090) Layout+Zen. Agent/Editor C(3069)/C(3070). Settings C(3066).
    Opener: getAnchor ()=>$, createInstance(G$_,...).render(U) (removes layer:-24).
#>
...
$Patches = @(
    ...
    @{
        Name                = "opener_no_layer_3012"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const G=AD(a,u)?_:void 0;return i.createInstance(G$_,s,o,a,l,u,d,m,g,p,f,i,y,G,N,k).render(U)}catch(G){throw G}}'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:U=>{try{const G=AD(a,u)?_:void 0;return i.createInstance(G$_,s,o,a,l,u,d,m,g,p,f,i,y,G,N,k).render(U)}catch(G){throw G}}'
    },
    @{
        Name                = "buildContent_full_3012"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_3012"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(3066,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(ZM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(ZM);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(ZM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (3.0.12) ===" "Cyan"
