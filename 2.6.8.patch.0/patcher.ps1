<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.6.8 - Restores the full "Change Layout" menu
    (Agent, Editor, Zen, Browser + all toggles).

.DESCRIPTION
    Patches the workbench.desktop.main.js in-place for Cursor 2.6.8 (stable).
    Restores Zen/Browser presets and individual toggle switches.
    2.6.8 uses: ayw (2 layouts), K2a (4 layouts), nh, wC, Si, qP, hyw.

.PARAMETER Restore
    Restore the backup instead of applying the patch.

.PARAMETER Path
    Custom path to Cursor installation.
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:H=>{try{const W=UF(a,u)?C:void 0;return i.createInstance(hyw'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:H=>{try{const W=UF(a,u)?C:void 0;return i.createInstance(hyw'
    },
    @{
        Name                = "buildContent_full_268"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Menu Patcher (2.6.8) ===" "Cyan"
    Write-ColorOutput ""
    ...
    if ($broken.Count -gt 0) {
        ...
        Write-ColorOutput "Your Cursor version may differ from 2.6.8." "Yellow"
        if ($patchesToApply.Count -eq 0) {
            if ($alreadyApplied.Count -gt 0) {
                Write-ColorOutput "All found patches are already applied." "Green"
            } else {
                Write-ColorOutput "ERROR: No patches can be applied. Use the patcher for your Cursor version." "Red"
                exit 1
            }
        }
    }
    ...
    try {
        ...
        foreach ($folder in $cacheFolders) {
            ...
            if (Test-Path $cachePath) {
                try {
                    ...
                    Write-ColorOutput "Warning: Could not clear $folder" "Yellow"
                }
            }
        }
        ...
        Write-ColorOutput "  - 4 presets: Agent, Editor, Zen, Browser" "Cyan"
        Write-ColorOutput "  - Toggles: Agents, Chat, Editors, Panel, Sidebar" "Cyan"
        Write-ColorOutput "  - Agent Sidebar, Inline Diffs, Status Bar" "Cyan"
