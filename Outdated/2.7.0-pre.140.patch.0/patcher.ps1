<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.140 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.140.
    pre.140 variables: jS_ (2 layouts), Ch (clear), rM (layoutEnum), i2 (cmd), ZS_ (createInstance), wi (StorageScope), MD (openerCheck).
    Note: Uses S() for i18n, getSidebarLocation() in renderModeGrid.
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:q=>{try{const J=MD(a,u)?_:void 0;return i.createInstance(ZS_'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:q=>{try{const J=MD(a,u)?_:void 0;return i.createInstance(ZS_'
    },
    @{
        Name                = "buildContent_full_pre140"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.140) ===" "Cyan"
